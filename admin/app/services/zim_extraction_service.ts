...
export class ZIMExtractionService {
  ...
  async extractZIMContent(
    ...
    try {
      ...
      logger.info(
        `[ZIMExtractionService]: Archive metadata - Title: ${archiveMetadata.title}, Language: ${archiveMetadata.language}`
      )
      ...
      for (const entry of archive.iterByPath()) {
        ...
        logger.debug(
          `[ZIMExtractionService]: Chosen chunking strategy for path ${entry.path}: ${strategy}`
        )
        ...
        logger.debug(
          `Extracted ${chunks.length} chunks from article at path: ${entry.path} using strategy: ${strategy}`
        )

        const nonEmptyChunks = chunks.filter((c) => c.text.trim().length > 0)
        logger.debug(
          `After filtering empty chunks, ${nonEmptyChunks.length} chunks remain for article at path: ${entry.path}`
        )
      ...
      logger.info(
        `[ZIMExtractionService]: Completed processing ZIM file. Total articles processed: ${articlesProcessed}`
      )
      logger.debug(
        'Final structured content sample:',
        toReturn.slice(0, 3).map((c) => ({
